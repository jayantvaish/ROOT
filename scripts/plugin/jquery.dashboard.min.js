...
(function (a) {
    a.fn.dashboard = function (m) {
        ...
        k.addWidget = function (r, p, accessible) {
			...
			{		
					...
					$('#messageDialog').html('<a style="font-family: verdana;font-size: 13px;">'+defaults.chartNotAccessibleMessage+'</a>');
					$('#messageDialog').dialog('open');
					return false;
			}
			else
			{			
						if(parseInt(jQuery.inArray(r.url,widgetArr))==-1)
						{
							widgetArr[count] = r.url;
							count++;
						...
						{
							$('#messageDialog').dialog( "option", "title", defaults.warningDialogTitle );
							$('#messageDialog').html('<a style="font-family: verdana;font-size: 13px;">Widget '+r.title+' is already added</a>');
							$('#messageDialog').dialog('open');
						}
					}
			};
        ...
        a("#" + k.id + " ." + b.widgetClass).live("widgetDelete", function (n, p) {
            //if (confirm(b.deleteConfirmMessage)) {
                ...
                widgetArr.splice($.inArray(p.widget.url, widgetArr),1);
                p.widget.remove()
            //}
        });
        ...
        k.element.live("addWidgetDialogSelectCategory", function (o, n) {
		    ...
		    //If the widget is lready added in dashboard, disable that widget to add again..
		    if(JSON.stringify(dsState).indexOf(t.url) >= 0) {
			disableWidget($('#addwidget'+t.id));
		    }
                })
            });
	...
	$('#tabs').tabs({
		...
		k = currentDashboard;
		reDrawTabsData(currentDashboard);
	    }
	}); 
